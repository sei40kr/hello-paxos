...
class Proposer:
    def __init__(self, acceptors: List[Acceptor]) -> None:
        ...
        self.preparing_acceptors: List[Acceptor] = []
  ...
  Send preparation requests to all the acceptors.
        ...
        for acceptor in self.acceptors:
            print('Send a preparation request to ' + str(acceptor) +
                  ': number=' + str(self.number))

            try:
                response = acceptor.prepare(self.number)
            except IOError:
                # Ignore the acceptors that didn't accept the preparation
                # request.
                print('Failed to send a preparation request to ' +
                      str(acceptor) + '.')
                continue
            ...
            if max_number < a_number:
                max_number = a_number
                # Remember the proposal value from the highest numbered
                # acceptor.
                self.value = a_value
